...
clear
if [ "$(id -u)" != "0" ]; then
 	printf "\033[31mCHƯA CÓ QUYỀN ROOT\033[0m\n"
 	exit
 else
	printf "\033[32mĐÃ CÓ ĐƯỢC QUYỀN ROOT\033[0m\n"
fi 

# Chào mừng
echo "----------------------------------------"
printf "Chào mừng đến với Semi-Restore cho \033[35miOS 10.3.x\033[0m của \033[33mDevNTNghia!\033[0m\n"
echo "Cảm ơn bạn đã tin tưởng dùng sản phẩm của mình!"
read -p "Nhấn RETURN để tiếp tục."

# Các thay đổi
clear
printf "\033[33m---Các thay đổi có trong bản 1.1:\033[0m\n"
echo "- Thêm màu vào để Semi-Restore dễ dùng hơn."
echo "- Xóa những lệnh gây xung đột."
echo "- Đẩy nhanh tiến độ xóa Jailbreak."
echo "- Thêm phần mở rộng cho việc chạy lệnh được tốt hơn: Core Utilites để có thể xác thực bạn đang là ROOT hay MOBILE."
echo "- Fix lỗi không thể xóa Source."
printf "\033[33m--- Hỗ trợ thêm các Tweak sau:\033[0m\n"
...
# Lưu ý
clear
printf "\033[33mXin bạn hãy bỏ ra 2-5 phút để đọc những lưu ý này, vì nếu có rủi ro do bạn không đọc, bạn sẽ phải chịu hậu quả.\033[0m\n"
echo "----------------------------------------"
printf "\033[31m--- Các lưu ý cần phải đọc kĩ trước khi sử dụng:\033[0m\n"
printf "\033[31m- Semi-Restore được làm ra từ file\033[0m \033[32mBOOTSTRAP\033[0m \033[31mcủa g0blin nên doubleH3lix và 32bit có thể không hoạt động được.\033[0m\n"
...
printf "\033[33m- Hãy để Tự động khóa về 0 giây để không bị ảnh hưởng đến việc xóa Jailbreak.\033[0m\n"
printf "\033[33m- Khi lệnh chạy xong, máy sẽ tự khởi động lại và trở về trạng thái như chưa từng Jailbreak.\033[0m\n"
printf "\033[36m-- Lưu ý ĐẶC BIỆT:\n"
printf "\033[36m- Chỉ nên cài từ nguồn\033[0m \033[32mdevntnghia.yourepo.com\033[0m \033[36mđể được update thường xuyên nhất.\033[0m\n"
printf "\033[36m- Hãy là 1 con người văn minh, đừng cướp đi sức lao động của người khác và quyền tác giả của họ.\033[0m\n"
...
# Chuẩn bị
printf "\033[33mXin bạn hãy chờ 3s để chuẩn bị lệnh...\033[0m\n"
sleep 3
echo ""
printf "\033[32mĐã chuẩn bị xong lệnh.\033[0m\n"
read -p "Nhấn RETURN để xác nhận việc xóa Jailbreak!"
clear

## Code
# Xóa file từ g0blin
echo "----------------------------------------"
printf "\033[31mBẮT ĐẦU VIỆC XÓA JAILBEAK...\033[0m\n"
sleep 1
...
# Xóa phần mở rộng của Tweaks
echo "----------------------------------------"
printf "\033[33mBây giờ sẽ tiến hành việc xóa các Tweak hỗ trợ, xin bạn hãy kiên nhẫn.\033[0m\n"
sleep 1
...
# Xóa Bootstrap
printf "\033[31mĐang xóa Cydia Bootstrap...\033[0m\n"
sleep 1
printf "\033[36mHÃY CẦU NGUYỆN NÓ THÀNH CÔNG...\033[0m\n"
...
echo "........................................"
sleep 5
printf "\033[32mĐã xóa xong Cydia Bootstrap.\033[0m\n"
printf "\033[33mBạn hãy chờ để lưu thay đổi.\033[0m\n"
/usr/bin/uicache
